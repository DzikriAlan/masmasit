...
Every id below was copied from a database that has since been reset, so on any
fresh project these rows have no parent to attach to. The `WHERE NOT EXISTS`
guards were only ever idempotency guards -- they do not stop a foreign key
violation -- so this file aborted a clean replay of the migration history.

Each statement now also checks that its parent row is actually present, which
makes the whole file a no-op on a database that never held the original data.
The current seed is migration 017.
*/
