...
export default function DashboardPage() {
  ...
  const [stats, setStats] = useState({ applications: 0, projects: 0, enrollments: 0, rsvps: 0 });
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [{ count: apps }, { count: projs }, { count: enrolls }, { count: rsvps }] = await Promise.all([
      ...
      setIsAdmin(roles.includes('super_admin') || roles.includes('regional_admin'));
    })();
  }, [user, roles]);
