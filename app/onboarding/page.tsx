...
export default function OnboardingPage() {
  ...
  const saveSkills = async () => {
    if (!user) return;
    setSaving(true);
    const { error: delErr } = await supabase.from('user_skills').delete().eq('user_id', user.id);
    if (delErr) {
      setSaving(false);
  ...
  const saveExperiences = async () => {
    if (!user) return;
    setSaving(true);
    const { error: delErr } = await supabase.from('experiences').delete().eq('user_id', user.id);
    if (delErr) {
      setSaving(false);
