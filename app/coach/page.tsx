...
export default function CoachPage() {
  ...
  const loadCourses = useCallback(async () => {
    ...
    const sorted = (data as CourseWithDetails[])?.map((c) => ({
      ...c,
      course_modules: c.course_modules?.sort((a, b) => a.order_index - b.order_index),
    }));
    setCourses(sorted ?? []);
    if (selectedCourse) {
      const updated = sorted?.find((c) => c.id === selectedCourse.id);
  ...
  const applyCoach = async () => {
    if (!user) return;
    setSaving(true);
    await supabase.from('profiles').update({ is_coach: true, coach_approved: 'pending' }).eq('id', user.id);
    setSaving(false);
  ...
  const createCourse = async () => {
    ...
    setSaving(false);
    if (error) { toast.error(t('Failed to create course', 'Gagal membuat kursus')); return; }
  ...
  const createModule = async () => {
    ...
    const orderIndex = (selectedCourse.course_modules?.length ?? 0);
    ...
    if (error) { toast.error(t('Failed to create module', 'Gagal membuat modul')); return; }
  ...
  const createMaterial = async () => {
    ...
    if (error) { toast.error(t('Failed to add material', 'Gagal menambahkan materi')); return; }
  ...
  const deleteMaterial = async (materialId: string) => {
    ...
    if (error) { toast.error('Failed to delete material'); return; }
  ...
  const createQuiz = async () => {
    ...
    setSavingQuiz(false);
    if (error) { toast.error(t('Failed to create quiz', 'Gagal membuat kuis')); return; }
  ...
  const deleteQuiz = async (quizId: string) => {
    ...
    if (error) { toast.error('Failed to delete quiz'); return; }
    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
          <Button variant="ghost" onClick={() => setSelectedCourse(null)} className="mb-4 gap-2">
                ...
                <Card key={mod.id} className="glass">
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => deleteModule(mod.id)} className="text-destructive hover:text-destructive">
                            ...
                            <div className="space-y-2">
                              {mod.course_materials.map((mat) => (
                                <div key={mat.id} className="flex items-center gap-2 rounded-lg border border-border/40 p-2">
                                  ...
                                  <Button size="sm" variant="ghost" onClick={() => deleteMaterial(mat.id)} className="text-destructive hover:text-destructive h-7 w-7 p-0">
                              ...
                              {mod.quizzes.map((qz) => (
                                <div key={qz.id} className="rounded-lg border border-border/40 p-3">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <Button size="sm" variant="ghost" onClick={() => deleteQuiz(qz.id)} className="text-destructive hover:text-destructive h-7 w-7 p-0">
                                      <Trash2 className="h-3 w-3" />
                                    </Button>
                                  </div>
                                  <div className="mt-2 space-y-1">
                                    {qz.quiz_questions?.map((qq, qi) => (
                                      <div key={qq.id} className="flex items-start gap-2 rounded border border-border/30 p-2 text-xs">
                                        ...
                                        <Button size="sm" variant="ghost" onClick={() => deleteQuestion(qq.id)} className="text-destructive hover:text-destructive h-5 w-5 p-0">
                                          <X className="h-3 w-3" />
                                        </Button>
                                      </div>
                                    ))}
        ...
        <Dialog open={showModuleDialog} onOpenChange={setShowModuleDialog}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button onClick={createModule} disabled={savingModule || !moduleForm.title} className="gap-2">
        ...
        <Dialog open={showMaterialDialog} onOpenChange={setShowMaterialDialog}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button onClick={createMaterial} disabled={savingMaterial || !materialForm.title} className="gap-2">
        ...
        <Dialog open={showQuizDialog} onOpenChange={setShowQuizDialog}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button onClick={createQuiz} disabled={savingQuiz || !quizForm.title} className="gap-2">
        ...
        <Dialog open={showQuestionDialog} onOpenChange={setShowQuestionDialog}>
          <DialogContent className="sm:max-w-lg">
            ...
            <DialogFooter>
              ...
              <Button onClick={createQuestion} disabled={savingQuestion || !questionForm.question || !questionForm.option_a || !questionForm.option_b} className="gap-2">
        ...
        {!profile?.is_coach ? (
          <Card className="glass">
            <CardContent className="p-8 text-center">
              ...
              <Button onClick={applyCoach} disabled={saving} className="mt-4 gap-2">
          ...
          <>
              ...
              <Card className="glass mb-6">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button onClick={createCourse} disabled={saving || !form.title || !form.description} className="gap-2">
            ...
            {courses.length === 0 ? (
              ...
              <div className="space-y-3">
                {courses.map((c) => (
                  <Card key={c.id} className="glass group cursor-pointer transition-all hover:border-primary/40 hover:-translate-y-0.5" onClick={() => setSelectedCourse(c)}>
